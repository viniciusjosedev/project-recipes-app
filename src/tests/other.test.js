import { fetchCategores, fetchDetails } from '../services/foodAndDrink';
import { getRecomendations } from '../helpers/ingredients';
...
describe('', () => {
  it('', async () => {
    ...
    expect(await fetchCategores('Shake', '/drinks')).toBeDefined();
    expect(await fetchCategores('Beef', '/meals')).toBeDefined();

    expect(await fetchDetails('drinks', '17222')).toBeDefined();

    expect(await getRecomendations('meals')).toBeDefined();
    expect(await getRecomendations('drinks')).toBeDefined();

    addFavoriteRecipes('Meal', favoriteRecipesMeals);
    addFavoriteRecipes('Drink', favoriteRecipesDrinks);
    favoriteRecipesDrinks.strAlcoholic = null;
    addFavoriteRecipes('Drink', favoriteRecipesDrinks);
  });
});
